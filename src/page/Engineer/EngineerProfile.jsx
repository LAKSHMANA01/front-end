...
useEffect(() => {
  ...
  const toggleAvailability = (day) => {
    setEngineer({
      ...engineer,
      availability: { ...engineer.availability, [day]: !engineer.availability[day] },
    });
  };
          ...
          <div className="p-6">
            {activeTab === "personal" && (
              <div className="grid md:grid-cols-2 gap-6">
                ...
                {/* Availability Section - Shown as Checkboxes */}
                <div className="md:col-span-2">
                  <h3 className="text-sm font-medium text-gray-700">Availability</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
                    {Object.keys(engineer.availability).map((day) => (
                      <label key={day} className="flex items-center space-x-2">
                        <input type="checkbox" checked={engineer.availability[day]} className="w-4 h-4" disabled />
                ...
                {/* Specialization Section - Shown as Radio Buttons */}
                <div className="md:col-span-2">
                  <h3 className="text-sm font-medium text-gray-700">Specialization</h3>
                  <div className="flex gap-4 mt-2">
                    {["Installation", "Fault"].map((spec) => (
                      <label key={spec} className="flex items-center space-x-2">
                        <input type="radio" name="specialization" value={spec} checked={engineer.specialization === spec} className="w-4 h-4" disabled />
                    ...
                    {["Installation", "Fault"].map((spec) => (
                      <label key={spec} className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) => setEngineer({ ...engineer, specialization: e.target.value })}
                          className="w-4 h-4"
                        />
                        <span className="text-sm">{spec}</span>
                      </label>
